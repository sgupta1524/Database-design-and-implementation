...
sp_register: BEGIN
    ...
    IF length(i_password) >= 8 AND (i_balance > 0 OR i_email IS NOT NULL) 
    AND (i_username NOT IN (SELECT username FROM `User`) 
    AND (CONCAT(i_firstname,' ',i_lastname) NOT in (SELECT CONCAT(firstName,' ',lastName) FROM `User`))) THEN 
        INSERT INTO cs4400spring2020.`User`(username,`password`,firstname,lastname)
        VALUES (i_username, i_password, i_firstname, i_lastname);
        UPDATE cs4400spring2020.`User` SET `password` = md5(`password`);
        IF i_email IS NOT NULL AND i_email <> '' THEN 
            INSERT INTO Employee(username,email) VALUES (i_username, i_email);
            IF i_type = 'Admin' THEN 
                INSERT INTO cs4400spring2020.`Admin`(username) VALUES (i_username);
            ELSEIF i_type = 'Manager' THEN 
                INSERT INTO Manager(username) VALUES (i_username);
            ELSEIF i_type = 'Staff' THEN 
                INSERT INTO Staff(username) VALUES (i_username);
            END IF;
		END IF;
        IF i_balance > 0 THEN
            INSERT INTO Customer(username,balance) VALUES (i_username, i_balance);
        END IF;
	ELSEIF length(i_password) < 8 OR i_balance <=0 OR (i_balance is NULL AND (i_email IS NULL OR i_email = '')) THEN
		LEAVE sp_register;
    END IF;
...
BEGIN
    ...
    (station.capacity - (SELECT COUNT(*) FROM foodtruck WHERE foodtruck.stationName=STATION.stationName) ) AS remainingCapacity ,
    COUNT(DISTINCT staff.username) AS staffCount, COUNT(DISTINCT foodName) AS menuItemCount
    FROM FoodTruck
    INNER JOIN STATION
    ...
    (FoodTruck.foodTruckName LIKE CONCAT('%',i_foodTruckName,'%') OR i_foodTruckName = "") AND
    (i_stationName = FoodTruck.stationName OR i_stationName = "") AND
    ((i_hasRemainingCapacity = TRUE AND (station.capacity - (SELECT COUNT(*) FROM foodtruck WHERE foodtruck.stationName=STATION.stationName) ) >0) OR (i_hasRemainingCapacity = FALSE))
...
BEGIN
    ...
    WHERE MenuItem.foodTruckName IN 
    (SELECT foodTruckName FROM FoodTruck
...
BEGIN
	...
	IF (SELECT foodTruckName FROM STAFF WHERE i_staffName = username) IS NULL THEN
		UPDATE STAFF
...
DELIMITER //
CREATE PROCEDURE mn_create_foodTruck_add_MenuItem(IN i_foodTruckName VARCHAR(50), IN i_price DECIMAL(6,2), IN i_foodName VARCHAR(50))
BEGIN
...
DELIMITER //
CREATE PROCEDURE mn_update_foodTruck_MenuItem(IN i_foodTruckName VARCHAR(50), IN i_price DECIMAL(6,2), IN i_foodName VARCHAR(50))
BEGIN
...
BEGIN
    ...
    SELECT customer.stationName, Station.buildingName,
            GROUP_CONCAT(DISTINCT(BuildingTag.tag) SEPARATOR ',') as tags,
            building.`description`, customer.balance
    from customer
    inner join station on station.stationName = customer.stationName
    inner join buildingtag on buildingtag.buildingName = station.buildingName
    inner join building on building.buildingName = station.buildingName
    where customer.username = i_customerUsername
    group by buildingtag.buildingName;
...
BEGIN
    ...
    select foodtruck.foodTruckName, concat(`user`.firstName,' ',`user`.lastName) as manager, GROUP_CONCAT(menuitem.foodName SEPARATOR ',') as food      
    from customer
    inner join foodtruck on foodtruck.stationName = customer.stationName
...
BEGIN
    ...
    IF i_customerUsername IN (SELECT username FROM customer) THEN
		SET @maxOrderId := (SELECT max(orderID)+1 FROM orders);
		INSERT INTO orders(orderID, date, customerUsername)
		VALUES (@maxOrderId, i_date, i_customerUsername);
	END IF;
...
BEGIN
	SET @purchasePrice := i_purchaseQuantity * (SELECT price FROM MenuItem WHERE MenuItem.foodName = i_foodName and MenuItem.foodTruckName = i_foodTruckName);
    SET @initialBalance := (SELECT balance FROM Customer WHERE Customer.username = (SELECT customerUsername from Orders WHERE Orders.orderID = i_orderId));
	IF (@purchasePrice <= @initialBalance)
    THEN
    
    UPDATE Customer
    SET Customer.balance = @initialBalance - @purchasePrice
    WHERE Customer.username = (SELECT customerUsername from Orders WHERE Orders.orderID = i_orderId);
    
	INSERT INTO OrderDetail(orderId, foodTruckName, foodName, purchaseQuantity)
	VALUES (i_orderId, i_foodTruckName, i_foodName, i_purchaseQuantity); END IF;
...
BEGIN
    ...
    SELECT Orders.`date`, Orders.orderID as orderID, 
    SUM(price*purchaseQuantity) as orderTotal, GROUP_CONCAT(DISTINCT OrderDetail.foodName SEPARATOR ',') as foodNames, SUM(purchaseQuantity) as foodQuantity
    FROM Orders
